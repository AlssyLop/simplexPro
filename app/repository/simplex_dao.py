import aiosqlite
import json
from app.schemas.simplex_model import ProblemaPL, Resultado, MostrarResultadoSimplex
...
async def mostrar_resultado_simplex(db: aiosqlite.Connection, id: str) -> Optional[MostrarResultadoSimplex]:
    ...
    return MostrarResultadoSimplex(
        id = id,
        fechaCreacion = "", # Not fetched but required by ResumenProblema
        titulo = resultado[0],
        ...
        valor_fo = json.loads(resultado[4]) if resultado[4].startswith('"') else resultado[4],
        mensaje = resultado[5],
        iteraciones = iteraciones
    )

async def actualizar_resultado_simplex(db: aiosqlite.Connection, problema_id: str, resultado: Resultado):
    valor_fo = json.dumps(resultado.valor_fo)
    mensaje = resultado.mensaje
